...
def task_two(video_file):
    ...
    while True:
        ...
        if len(bounding_boxes) > 0:
            bounding_boxes = sorted(bounding_boxes, key=lambda x: x[4], reverse=True)[:3]
            closest_frame = frame.copy()
            for (x, y, w, h, _) in bounding_boxes:
                cv2.rectangle(closest_frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
                cv2.putText(closest_frame, "Closest", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        else:
            closest_frame = frame.copy()
        ...
        # 2: Detect pedestrians and overlay bounding boxes
        ...
        if len(indices) > 0:
            ...
            for i in indices:  # Iterate over each valid index
                ...
                found_id = None
                min_distance = 50  # Set a distance threshold
